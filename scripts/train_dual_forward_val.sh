...
CUDA_VISIBLE_DEVICES="${gpu_id}" python intermimic/run.py \
    ...
    --cfg_train "${cfg_train}" \
    --headless \
    --output_path "${out_root}/${seq_name}_dual/forward/ref_tar/ref_tar_${current_epoch}" \
    --stateInit Start \
    ...
    --checkpoint "${out_root}/${seq_name}_dual/forward/smplx/nn/${checkpoint_name}" \
    --hoi_refs_path "${out_root}/${seq_name}_dual/forward/ref_hoi/ref_hoi_${current_epoch}.npz" \
    --hoi_data_path "${out_root}/${seq_name}_dual/forward/hoi_data/intermimic_${current_epoch}.pt" \
